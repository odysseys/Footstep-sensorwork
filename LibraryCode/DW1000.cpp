...
const SPISettings DW1000Class::_fastSPI = SPISettings(2000000L, MSBFIRST, SPI_MODE0); // -cef
...
void DW1000Class::commitConfiguration() {
	...
	//writeValueToBytes(antennaDelayBytes, 16384, LEN_STAMP); //KENT (original delay)
	writeValueToBytes(antennaDelayBytes, 16350, LEN_STAMP); // KENT: 1.6m, DW1000.MODE_LONGDATA_FAST_ACCURACY
...
void DW1000Class::setDefaults() {
	if(_deviceMode == TX_MODE) {
		...
		useExtendedFrameLength(false); //-- Shijia
		// useExtendedFrameLength(true); // -- Shijia: use extended frame length for larger buffer
		useSmartPower(false);
		suppressFrameCheck(false);
        //for global frame filtering
		setFrameFilter(false); // true -- changed by Shijia
