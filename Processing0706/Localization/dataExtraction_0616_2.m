...
s1 = s1(4400000:5300000);
s2 = s2(4400000:5300000);
s3 = s3(4400000:5300000);
s4 = s4(4400000:5300000);
...
%% alignment
% find first shared timestamp
unifiedTime = [timestamp1; timestamp2; timestamp3; timestamp4];
unifiedTime = unique(unifiedTime);
for i = 1 : length(unifiedTime)
    if ismember(unifiedTime(i),timestamp1) ...
        && ismember(unifiedTime(i),timestamp2) ...
        && ismember(unifiedTime(i),timestamp3) ...
        && ismember(unifiedTime(i),timestamp4) 
        baseline = unifiedTime(i);
        break;
    end
end
% baseline = max([timestamp1(1),timestamp2(1),timestamp3(1),timestamp4(1)]);
endline = min([timestamp1(end),timestamp2(end),timestamp3(end),timestamp4(end)]);
cuttingPoint1 = timestampIdx1(timestamp1 == baseline);
cuttingPoint2 = timestampIdx2(timestamp2 == baseline);
cuttingPoint3 = timestampIdx3(timestamp3 == baseline);
cuttingPoint4 = timestampIdx4(timestamp4 == baseline);

endPoint1 = timestampIdx1(timestamp1 == endline);
endPoint2 = timestampIdx2(timestamp2 == endline);
endPoint3 = timestampIdx3(timestamp3 == endline);
endPoint4 = timestampIdx4(timestamp4 == endline);
...
%     end
    ...
    if maxIdx > length(s1_vib) || maxIdx > length(s2_vib) || maxIdx > length(s3_vib) || maxIdx > length(s4_vib)
        continue;
    end
    if isempty(temp1)
        temp1 = maxIdx;
